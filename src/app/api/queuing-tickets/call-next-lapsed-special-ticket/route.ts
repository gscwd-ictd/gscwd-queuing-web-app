...
export async function PUT() {
  ...
  try {
    ...
    const updatedTicket = await prisma.$transaction(async (tx) => {
      const lastLapsedTicket = await tx.queuingTicket.findFirst({
        where: {
          transactionId: counter.transactionId,
      ...
      let nextLapsedTicket = await tx.queuingTicket.findFirst({
        where: {
          transactionId: counter.transactionId,
          queuingStatus: QueuingStatus.LAPSED,
          isPrioritized: true,
          dateCreated: { gte: startOfDay, lte: endOfDay },
          position: {
            gt: lastLapsedTicket?.position ?? 0,
          },
        },
      ...
      if (!nextLapsedTicket) {
        nextLapsedTicket = await tx.queuingTicket.findFirst({
          where: {
            transactionId: counter.transactionId,
            queuingStatus: QueuingStatus.LAPSED,
            isPrioritized: true,
      ...
      if (!nextLapsedTicket) return null;

      const updatedTicket = await tx.queuingTicket.update({
        where: { id: nextLapsedTicket.id },
        data: {
